...
new_preference = [5, 3, 9, 14, 25, 99, 54]

users_count = len(preferences)  # 158 elements
training_count = int(0.7 * users_count)

random.shuffle(preferences)
training_preferences = preferences[:training_count]
testing_preferences = preferences[training_count:]

knn = KNearestNeighbours()
knn.fit(training_preferences)
predictions = knn.predict(new_preference)

distances = list(map(lambda x: x[0], predictions))
